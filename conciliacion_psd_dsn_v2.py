...
st.write('Herramienta para identificar **DSN (Depósitos Sin Notificación)** y **PSD (Pagos Sin Depósito)** entre EECC del banco y registros de Metabase.')
st.divider()

# ----------------------------
# Cargar archivos
# ----------------------------
archivo_eecc = st.file_uploader('📥 Subir EECC del banco', type=['xlsx', 'xls'])
...
if archivo_eecc is not None and archivo_metabase is not None:
    ...
    # Índices de columnas (ajusta si cambia la posición)
    ...
    if col_eecc_index >= len(df_filtrado.columns) or col_meta_index >= len(data_metabase.columns):
        ...
        # -----------------------
        # 🔁 PSD: en Metabase pero no en EECC
        # -----------------------
        st.subheader('🔁 PSD (Pagos Sin Depósito)')
        psd = data_metabase[~data_metabase[col_meta].isin(df_filtrado[col_eecc])]
