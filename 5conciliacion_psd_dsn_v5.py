...
def cargar_txt_crep(archivo_txt):
    ...
    for linea in lineas:
        if linea.startswith('DD'):
            try:
                psp_tin_raw = linea[111:123].strip()
                psp_tin = psp_tin_raw.lstrip('0')

                monto_raw = linea[63:76].strip()
                monto = int(monto_raw) / 100

                medio_atencion = linea[123:134].strip()

                fecha_raw = linea[47:55]
                fecha_pago = f"{fecha_raw[6:8]}/{fecha_raw[4:6]}/{fecha_raw[0:4]}"

                hora_raw = linea[55:61]
                hora_pago = f"{hora_raw[0:2]}:{hora_raw[2:4]}:{hora_raw[4:6]}"
                ...
                registros.append({
                    ...
                    'Nº operación': psp_tin  # para compatibilidad con la lógica
                })
